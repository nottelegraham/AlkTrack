 ...
 *  WHAT IS NEW IN v2.2:
 *
 *  CHANGE 1 — TWO-PUMP ARCHITECTURE:
 *    v2.1 required three pumps: Pump A (reversible, reservoir),
 *    Pump B (one-way, tank in), and Pump C (one-way, waste drain).
 *    v2.2 removes Pump C entirely. Pump B is now reversible and
 *    returns tank water to the sump after each measurement.
 *    No waste line or waste container required.
 *    Relay wiring: IN1=Pump A fwd, IN2=Pump A rev,
 *                  IN3=Pump B fwd, IN4=Pump B rev.
 *    GPIO assignments unchanged — GPIO 5 was PUMP_WASTE,
 *    is now PUMP_TANK_REV. Same physical relay channel,
 *    new logical function.
 *
 *  WHAT THIS CARRIED FORWARD FROM v2.1:
 *
 ...
 *    PUMP_TANK_FWD, PUMP_TANK_REV, and AIR_PUMP are assigned here.
...
const int PUMP_TANK_FWD = 18;  // Pump B fwd: tank -> chamber
const int PUMP_TANK_REV = 5;   // Pump B rev: chamber -> tank
...
void setup() {
  ...
  const int outPins[] = {PUMP_REF_FWD, PUMP_REF_REV, PUMP_TANK_FWD,
                         PUMP_TANK_REV, AIR_PUMP, DAC_TREND_OUT};
  ...
  Serial.println("Ready. Commands: CAL <dKH>  STATUS  PUMPS_OFF  RUN  PRIME");
}
...
void runFlushCycle(bool doubleRinse) {
  ...
  for (int i = 0; i < cycles && !g_abortCycle; i++) {
    ...
    runPumpBlocking(PUMP_TANK_FWD, RINSE_FILL_MS);
    runPumpBlocking(PUMP_TANK_REV, RINSE_DRAIN_MS);
  }
}

SampleReading runTankCycle() {
  Serial.println("[TANK] Filling from sump...");
  runPumpBlocking(PUMP_TANK_FWD, FILL_TIME_MS);
  runAeration(AERATE_TIME_MS);
  if (g_abortCycle) { runPumpBlocking(PUMP_TANK_REV, DRAIN_TIME_MS); return {0, 0, 0, false}; }
  runSettleDelay(SETTLE_TIME_MS);
  if (g_abortCycle) { runPumpBlocking(PUMP_TANK_REV, DRAIN_TIME_MS); return {0, 0, 0, false}; }
  SampleReading r = readAveragedSample("TANK");
  Serial.println("[TANK] Returning water to sump...");
  runPumpBlocking(PUMP_TANK_REV, DRAIN_TIME_MS);
  return r;
}
...
void ensurePumpsOff() {
  ...
  digitalWrite(PUMP_TANK_FWD, LOW);
  digitalWrite(PUMP_TANK_REV, LOW);
  digitalWrite(AIR_PUMP,     LOW);
}
...
void handleSerialCommands() {
  ...
  if (line.startsWith("CAL ")) {
    ...
    // Runs each pump long enough to push air out of all tubing on first install.
    // Safe to repeat any time — chamber drains at the end.
    // Do NOT run while a measurement cycle is active.
    Serial.println("PRIME: Priming all tubing lines...");
    Serial.println("  Step 1/4 — Ref reservoir line (Pump A fwd, 30 s)");
    runPumpBlocking(PUMP_REF_FWD, 30000UL);
    Serial.println("  Step 2/4 — Return ref water to reservoir (Pump A rev, 30 s)");
    runPumpBlocking(PUMP_REF_REV, 30000UL);
    Serial.println("  Step 3/4 — Tank line fwd: sump to chamber (Pump B, 20 s)");
    runPumpBlocking(PUMP_TANK_FWD, 20000UL);
    Serial.println("  Step 4/4 — Tank line rev: chamber to sump (Pump B, 20 s)");
    runPumpBlocking(PUMP_TANK_REV, 20000UL);
    Serial.println("PRIME complete. Both pump lines now water-filled.");
    Serial.println("Verify no leaks, then send RUN or wait for the next cycle.");
  }
  else {
    Serial.println("Commands: CAL <dKH>  STATUS  PUMPS_OFF  RUN  PRIME");
  }
}
